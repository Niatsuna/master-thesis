...
This use-case demonstrates several key aspects of modern edge deployments:
\begin{itemize}
...
\textbf{Can Ecoscape effectively orchestrate resource-intensive LLM workload across heterogeneous edge computing environments?}
...
PyTorch\footnote{\url{https://pytorch.org/}} serves as the deep learning framework for model loading in the LLM service and has gained a widespread adoption in both research and production environments [\cite{paszke2019pytorchimperativestylehighperformance}].
...
It features around 52,000 instruction-following prompts ranging from tasks like questions answering, creative writing, summarization and reasoning.
...
As startup can have multiple causes (e.g. Scenario Start, Pod Crash), this overhead results in unnecessary delay based on the scenarios size and create a long living dependency on external services.
...
We therefore use Persistent Volume Claims (PVCs) with initialization jobs to preload our resource before deployment into the cluster.
...
This results in downloading each unique resource once rather than multiple times, decreasing our startup latency, scenario delay and dependency on external services.
...
To assess the effectiveness of our distributed LLM deployment on Ecoscape, we define several key metrics inside the LLM service.
...
\begin{itemize}
  \item \textit{node\_id} : Unique identifier for the node running the service
