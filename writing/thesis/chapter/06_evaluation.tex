...
The described preloading of the dataset and model were executed before the any scenario and are not part of the scenarios itself.
...
If not stated otherwise the used requests per second (RPS) and load pattern where $0.2$ ( 1 message every 5 seconds) and a poisson distribution respectively.
...
\textbf{Edge nodes} are resource-constrained with 2 CPU cores and 4 GiB of memeory, representing typical edge infrastructure.
...
Resource quests and limits are configured according to node type via kubernetes deployment and therefore ensure realistic edge computing constraints while preventing resource contention.
...
% ----------------------------------
\section{Experiments}
With the previous denoted setup, we present in this section general consensus in all executed experimental scenarios and highlight three selected scenarios from our broader suite of validation experiments conducted to assess Ecoscape's infrastructure capabilites for LLM deployment.
...
The designated 1-minute warm-up interval accomodates this variation across deployment scales.

% ----------------------------------
\subsection{Scenario 1: Resource Limit Validation}
\subsubsection{Objective \& Methodology}
The goal of scenario 1 is to validate that Ecoscape is accurately model its saturation limit for a single node.
...
We steadily increase every 5 minutes the amount of replicas by 1.

If Ecoscape accurately models linear scaling via replicas, than it is to be expected that the overall TTPS for the entire cluster should increase linearly with the number of replicas, while the individual replica's TTPS remain stable.
...
\subsubsection{Objective \& Methodology}
For our last highlighted scenario, we observe the actual performance difference between different node types to validate that Ecoscape accurately model the massive performance upgrade from specialized hardware like GPUs.
...
After further investigation, it was discovered that at these spikes multiple difficult and long prompt were present, resulting in long and novel responses.
...
One interesting discovery was the comparison between the CPU nodes, as CPU utilization didn't not adapt to more CPUs and TTFT was at some datapoints worse on the cloud node than the edge node.
...
\textbf{Workload pattern:} The Alpaca dataset provides diverse prompts, while our experiments often feature poission distribution, ramp up or constant as load patterns. Real-world deplyoments would benefit from additional expeiments with variable load profiles, realistic bursty traffic patterns and datasets with integrated time series to integrate data on more authentical level.
...
These limitations do not invalidate our findings but rather identify oppertunities for future work and usage of this use-case by extending it to more complex scenarios.
